...
function Discover({ go }: { go: (screen: Screen) => void }) {
  ...
  return <>
    ...
    <Surface className="glow-filter-card"><ChoiceGroup label="Date" value={when} options={['Upcoming', 'This week', 'This month']} onChange={setWhen} /><ChoiceGroup label="Format" value={format} options={['All', 'Singles', 'Doubles']} onChange={setFormat} /><details className="glow-search-disclosure"><summary><Search size={17} aria-hidden="true" />Search{query && <span> · Active</span>}</summary><input className="glow-search" aria-label="Search tournaments" placeholder="Tournament or place" value={query} onChange={e => setQuery(e.target.value)} /></details></Surface>
    <Surface className="glow-results"><strong>{matches.length} open tournaments</strong><span className="glow-muted">{when}</span></Surface>
